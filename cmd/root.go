...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.relay.yaml)")
...
func initConfig() {
	if cfgFile != "" {
		...
		// Search config in home directory with name ".relay" (without extension).
